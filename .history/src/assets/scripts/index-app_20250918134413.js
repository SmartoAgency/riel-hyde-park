...
document.addEventListener('DOMContentLoaded', function() {
  ...
  config.forEach(group => {
    const checkboxes = document.querySelectorAll(
      `input[type="checkbox"][name^="${group.checkboxPrefix}"]`,
    );
  ...
  function syncSelectWithHiddenInput(selectName) {
    const select = document.querySelector(`select[name="${selectName}"]`);
    const hiddenInput = document.querySelector(`input[type="hidden"][name="${selectName}"]`);
  ...
  syncSelectWithHiddenInput('for-whom');
  syncSelectWithHiddenInput('contact_way');
});
